...
- **銘柄マスタの起動時ソート（2026-08-06）**: 既定は **追加日 降順 → 更新日 降順 → コード 昇順** の3段（`SM_DEFAULT_SORT='_default'` ＋ `smDefaultCmp`。追加日/更新日が未設定の銘柄は末尾）。「最近追加/更新した銘柄が上に来る」ため、取込・登録直後の確認が速い。ヘッダをクリックすると従来どおりその列の単一キーソート（`sortValue`）に切り替わり、既定キーは列に無いのでソート矢印は出ない。ソート状態は端末内メモリのみ（リロードで既定に戻る）。
...
- **高値補完の毎回リトライ抑制（2026-07-24）**: 上記④の補完取得は成功日 `highsAt !== today()` で対象を拾うが、**Yahooが5年チャートを返せない銘柄（新形式コード・上場間もない・コード誤り等）は `highsAt` が永久に古いまま**で、株価更新のたびに毎回1件リトライされていた（「5年高値を取得中… 1/1件」が毎回・表の5年高値列は空欄のまま）。**連続失敗回数 `highsFail`（成功で0リセット）と試行日 `highsTriedAt` を新設**し、補完対象を「今日成功済みでない」かつ「`highsFail < 3` なら毎回／`>= 3` なら `highsTriedAt !== today()`（1日1回）」に変更。狙いは**両立**——(a) 一時的失敗（瞬断・レート制限）は数回まで毎回リトライして当日中に回復、(b) 新規/取込銘柄は `highsFail=0` で必ず初回に取得、(c) 恒久的に取れない銘柄だけ1日1回に抑制（翌日再試行＝ティッカー復活で拾う）。`highsFail`/`highsTriedAt` は `refreshPrice`（補完経路・常に highs=1）で毎回更新、`refreshAll` 主ループは `withHighs` の時のみ更新（highs を投げない通常更新は据え置き）。既存高値を null で潰さない方針は不変。
...
- **列「時価1位まで」＝市場の時価総額1位までの上昇余地（2026-08-06）**: 保有銘柄/銘柄マスタ/サインの列 `capToTop`。**同じ市場（日米別）の時価総額1位の時価総額 ÷ その銘柄の時価総額** を「◯.◯◯倍」で表示（例: 1位 5T・自分 1T → 5.00倍＝+400%の上昇余地）。**原則1倍を下回らない**（1位自身は1.00倍＝「1位」表示）。tooltip に1位の銘柄名・時価総額・上昇率(%)を出す。分子（市場の時価総額1位）は **`store.data.mktTopCap = { US/JP: {code,name,cap,at} }`（トップレベル・sync SCHEMA に `['map', byAt]` で登録）** に保持（**原通貨の実額**。US=USD / JP=円。`calc.marketCap` は百万単位なので比較時に ×1e6）。**当初 `settings` 配下に置いたが即バグ化したので移設した（2026-08-06）**——`settings` の同期は `singleTs`＝**オブジェクトまるごと後勝ち**なので、「端末が自動で毎日書き込む値」を入れると、スマホがアプリを開いただけで `settings._updatedAt` が更新され、**PCで変えた設定（画面の並び順 `navOrder` 等）を丸ごと巻き戻す**（実際に発生）。**教訓: `settings` に書いてよいのはユーザーの明示操作だけ。自動取得・派生キャッシュは独立キー＋キー単位マージにする。**更新経路は3つで、**主経路は描画時の自己補完 `ensureTopMarketCap`**（分子が未取得 or 前日以前なら `kind=marketcap&count=1` を市場ごと**1日1回**だけ取得→取れたら再描画。失敗しても当日は再試行しない＝毎描画リトライ防止）。残る2つは追加取得ゼロの相乗り——②株価更新時のランキング順位バッジ取得 `loadRankBadges`（`count=10` を日米とも1日1回取得済み）の1位、③マーケットタブで時価総額ランキング（全市場）を表示した時（`loadRanking`。**キャッシュ表示でも `syncTopCapFromCache` で反映**＝表示中ランキングと列の分子がズレない）。競合しても古い記録で新しい記録を上書きしないよう、`mktTopCap[market].at` に取得時刻(ms)を持たせて比較する。Google同期対象（`mktTopCap` を SCHEMA に登録済み。**`settings` には一切書かない**）。1位が未取得のうちは「—」（tooltip で取得方法を案内）。**配線**: `MASTER_COLS`／`COL_RENDERERS`／`DEFAULT_VISIBLE`(US/JP/MKTRANK)／`sortValue`／`cfCellValue`＋`CF_NUMERIC_KEYS`（背景色ルール可・倍率なので `CF_MONEY_KEYS` には入れない）／`colDefaultWidth`／銘柄詳細ドロワー。**表示場所（2026-08-06 追加）**: 保有銘柄・買い増しサインに加えて、①**銘柄マスタ**（固定列 `SM_COLS` に「時価1位まで」を追加。ヘッダクリックで `sortValue` ソート・背景色ルールも適用）、②**銘柄カルテ**のファンダボックス（時価総額の下に「時価総額1位まで ◯.◯◯倍（+◯%） 1位 ◯◯」）、③**マーケットのランキング**（列設定 `MKTRANK` の既定表示。**ハイブリッド列** `MKTRANK_HYBRID_KEYS` として実装＝ランキング取得値 `it.marketCap` から算出→無ければ登録済み銘柄の `calc.capToTop`）。米株ランキングは全種別（売買代金/値上がり等）で `marketCap` が付くので**未登録銘柄でも倍率が出る**。日本株ランキングは時価総額ランキング以外に時価総額が無いため、未登録は「—」・登録済みはツール内の値で補完。表示テキストは `capToTopFmt`／セルは `capToTopTd`／ランキング行用の算出は `capToTopOf` に共通化（4箇所で同じ見た目・同じtooltip）。**自動算出の派生値なので、銘柄編集フォーム・取込（GI/汎用往復）・一括変更・転記用には通さない**（手入力する項目ではないため）。
...
- **データモデル（store）**: `securities / holdings / transactions / rules / categories / investCategories / labelDefs / prices / fx / meta / indices / amountHistory / amountSnapshots / analyses / priceScenarios / importHistory / importMappings / matrixBands / matrixSettings`。
  ...
  - **推移グラフ**（2026-09-10 すみぽん指示・v20260910-1510）: 表の上にバニラSVGの推移グラフ（`renderTxnChart`/`txnChartSvg`）。買い＝青棒・売り＝橙棒（並列）・ネット（買い−売り）＝折れ線＋点（正=緑/負=赤）。区切り（`txnChartBuckets`）は期間トグル連動: **月別＝選択年月を末尾とする直近12ヶ月**／**年別＝最初の取引年〜今年（年単位・全期間）**／**全期間＝最初の取引月〜今月（月単位）**。絞り込み `txnFilter` はグラフにも効く。選択中の年/月は帯で強調。ホバーで区切りの買い/売り/ネット・件数、クリックでその区切りを選択（全期間では**その月の月別へ**ドリルダウン）。高さは「表＋注記が窓に収まる残り」（160〜340px）＋ `main.content` の実はみ出し量で自己補正（ルール6）。幅0（レイアウト未確定）時は rAF で再試行。**取引サマリーの初期表示は月別**（`reportPeriod='month'`）。
  ...
  - `priceScenarios`（**株価シナリオ分析の履歴**・2026-08-25）= `{ id, securityId, scenarioDate, scenarioPrice, stBear, stBase, stBull, mtBear, mtBase, mtBull, createdAt, updatedAt }`。**短期・中期×弱気/ベース/強気の予想株価レンジ**。自然キー＝`securityId|scenarioDate`（sync SCHEMA `ps:` records）。**銘柄分析(analyses)とは別の分析・別の日付体系なので別マスタ**（同じ analyses に混ぜると `syncLatestAnalysis`/`syncLatestScenario` が互いのフィールドを null で潰し合うため）。銘柄平置き（`scenarioDate/scenarioPrice/stBear...mtBull`）は `latestScenario` のミラー（analyses と同方式。`upsertScenario`→`syncLatestScenario`）。レンジは文字列で **`"1000~1500"` に正規化して保存**（`scNormRange`。入力は「1000～1500」「1,000-1,500」等を許容、単一値も可。数値化できない文字列はそのまま保持＝表示のみ）。派生値 **「現在値がどのレンジにいるか」＝`scPosition(sec,'st'|'mt')`**（弱気未満/弱気圏/弱気〜ベース/ベース圏/ベース〜強気/強気圏/強気超えの7段階。**レベル1〜7**の数値を持ち、タグは `SC_POS_COLORS`＝レベル別の既定色（red/orange/gold/blue/cyan/green/purple・`labelColorStyle`）、**セル背景はマスタの背景色ルールで変更可**＝`CF_NUMERIC_KEYS`/`cfCellValue` に `stScenPos`/`mtScenPos` を登録しレベル値で範囲指定。一部レンジ未入力でも「弱気=2/ベース=4/強気=6」の基準ランクからレベルを安定割当）。表のレンジ列は**改行なしの1行**「弱気 / ベース / 強気」を `/` 区切りで表示（縦並びは行が高くなりNG・2026-08-25 すみぽん指示）。**配線**: 銘柄編集フォーム「株価シナリオ」欄＋保存patch＋履歴upsert / 表の列（`stScenario`/`mtScenario`＝弱・ベ・強を1セル縦並び、`stScenPos`/`mtScenPos`＝位置タグ、`scenarioDate`/`scenarioPrice`。**既定非表示**・列設定でON。`sortValue` はレンジ列=ベース下限・位置列=下→上の序数）/ 取込（GI: `GI_FIELDS`グループ「株価シナリオ」・`GI_AUTOMAP`「分析日/分析時株価/短期弱気…」・`giParseValue`＋`runGenericImport` で履歴upsert／汎用往復: `GENERIC_MAP/HEADER` 末尾8列・`parseGeneric`・`genericFieldValue`＝正規化形式のまま往復）/ 詳細ドロワー「株価シナリオ」ボックス・銘柄カルテの評価分析ボックス / 分析履歴モーダルに「株価シナリオ履歴」表 / **分割調整 `applySplit` の対象**（分析時株価＋6レンジを平置き・履歴とも÷比率。履歴を調整しないと次のミラーで未調整値が戻るため）。**一括変更・転記用には通さない**（銘柄ごとに固有のレンジで一括設定・転記の用途がないため）。
...
> ただし **`transactions.skipPrevBuy = true` の買いは前回購入の候補から除外**する（下記）。

#### 前回購入から除外する取引（skipPrevBuy）
取引記録フォーム／取引履歴の一括取込の **「前回購入額を更新しない」** チェックで、その買い取引に
`skipPrevBuy: true` を付与できる。用途は「買ったが次回購入ラインの基準にはしたくない」買い
（打診買い・端株の買い増し・損出しの買い直し等）。

- **効かせる場所は1つだけ**: `calc.lastBuyInfo`（サーバーは `signal.js resolveLastBuy`）の
  買い取引の絞り込みで除外する。前回購入単価・前回購入日を使う全機能（次回購入トリガー＝addon基準・
  前回からの下落率・高値更新判定の日付比較・チャートの橙破線・列表示）はここを経由するため一括で効く。
- **効かない場所**: 保有数量・平均取得単価・取得円(acqJpy)・購入回数(buyCount)・取引サマリー。
  これらには通常の買いとして反映される（＝`ledgerOnly` とは直交する別フラグ。併用可）。
- **全ての買いが除外された場合**は、買い取引が無い時と同じフォールバック
  （手入力 `prevBuyPrice` → 保有の平均取得単価「みなし」）になる。みなしの平均取得単価には
  除外した買いも含まれる点に注意（完全に無視したい場合は「前回購入価格」を手入力する）。
- 取引編集で解除できるよう、`updateTransaction` は適用前に `skipPrevBuy` を `delete` する
  （`settleJpy` / `ledgerOnly` と同じ「フォーム送信値で完全上書き」方式）。
...
### 9.1 汎用取込の保存フォーマット（2026-07-29 修正）
`store.data.importFormats[] = {id, name, mapping:{列見出し→フィールドkey}, fixed:{固定値}}`。
- **保存は全列**。取込まない列は `''` を明示的に持たせる（「取込まないと決めた列」と「保存時に無かった列」を読込時に区別するため）。
- **読込は 3段階**: ①見出しの完全一致 → ②**正規化キー一致**（NFKC＋空白/★☆除去＋小文字化。`★リスク`と`リスク`、全角半角ゆれを吸収）→ ③どちらにも無い見出しは**自動対応（`giAutoField`）を残す**。
- **修正前の不具合**: 読込が `fmt.mapping[h] || ''` だけだったため、保存時と見出しの表記が少しでも違う列（`コード`↔`ティッカー`、`★リスク`↔`リスク` など）は自動対応ごと空に潰され、「保存したのに読み込むと一部の列だけリセットされる」状態になっていた。
- 適用結果は toast に「保存 n列 / 自動 m列」で出す（どこが自動割当かを気づけるように）。
...
- **3-wayマージ**（`sync-merge.js` の `SyncMerge.mergeBundle(base, local, remote)`）。base=前回同期時点を localStorage `sm_sync_base` に保持。
  ...
  - **`settings` は `keyedTs` 規則＝キー単位マージ（2026-08-06。旧 `singleTs`）**: 設定オブジェクトを**キー1つずつ3-way**で混ぜる。片側だけ変わったキーはその値（キーの削除も伝播）、**同じキーを両端末で変えた時だけ** `_updatedAt` の新しい方を採る。`_updatedAt` は新しい方を残す。旧 `singleTs`（オブジェクトまるごと後勝ち）は「PCで `navOrder`／スマホで別のキー」を変えただけで相手の変更を巻き戻し、**実際に画面の並び順が消えた**（スマホは `mktTopCap` の自動書き込みで settings が新しくなっていた）。`newsPrefs` も同じ理由で `keyedTs` 化。**新規登録では `single`/`singleTs` を選ばない**——使ってよいのは「全体で1つの意味を持つ塊」（`newsPool` のスナップショット・`listedMasterInfo` の取込メタ・`matrixSettings`）だけ（CLAUDE.md ルール7-2）。
...
- **Drive世代バックアップ（最大5世代・2026-06-11）**: 誤操作/不具合でのデータ消失対策。同じ `securities-manager` フォルダに `backup-YYYYMMDD-HHMMSS.json` を最大5世代保存（古いものから剪定。`data.json` 同期とは name 条件で非干渉）。作成タイミング＝**①1日1回（その日最初の同期で上書き前のDrive内容）②全データ削除/インポートの直前**。復元UI＝「バックアップ・出力」→「Driveのバックアップから復元…」。復元は `sm_sync_base` クリアで次回同期により反映。
  - **復元範囲の選択（部分復元・2026-08-27 すみぽん要望）**: 世代を選ぶと範囲選択モーダル（`restoreDriveBackup`→`execRestoreDriveBackup`）。`RESTORE_SCOPES` でトップレベルキーを6グループに分類——**①本体**（securities/holdings/transactions/acqLedger/amountHistory/amountSnapshots/analyses/priceScenarios/seq）**②マスタ**（rules/categories/investCategories/labelDefs/cfRules/grades/matrixBands/matrixSettings/discTypeDefs/ytChannels/newsTags/macroAlerts）**③設定**（settings/_colPrefs/_filterPresets/macroView/newsPrefs）**④取得キャッシュ**（prices/meta/earnings/fx/indices/techAnalysis/mktRanking/mktTopCap/macro/macroIdx/last系）**⑤ニュース・更新情報**（newsPool/newsRead/newsSeen/newsHidden/newsTrans/updates/updSeen/ytSummaries）**⑥取込関連**（listedMaster/listedMasterInfo/importMappings/importAliases/importFormats/importHistory）。チェックした範囲だけ `restoreFromBackup(fileId, scopeKeys)` がバックアップ値へ置き換え（バックアップに無いキーは削除＝当時の状態へ。load() が既定値を再シード）、未チェックは現在のまま。全選択＝従来の全体置換（未分類キー含む）。**新しいトップレベルキーを足したら RESTORE_SCOPES にも登録する**（未登録キーは部分復元で「現在のまま」＝安全側）。
...
- **同期の往復中に確定したローカル変更を巻き戻さない（2026-08-12・重要）**: `syncNow` は「①Drive読込 → ②`local = dataBundle()` スナップショット → ③マージ → ④**await でDrive書込** → ⑤`restoreBundle(merged)`」の順で動く。旧実装は⑤で `merged` をそのまま `store.data` に代入していたため、**②〜④の間（Driveの往復＝数百ms〜数秒）に確定したローカル変更が丸ごと消えていた**（read-modify-write レース）。実害＝**ニュースの「更新」で増えた記事が、少し経つと更新前の状態に戻る**（`newsRefresh` はRSS＋TDnet＋YouTubeで最大25秒かかり、自動同期25秒間隔と重なりやすい。さらに巻き戻った古い一覧が次の同期でDriveにも書かれる）。対策＝**④の後に現在のローカル(`_snapshot()`)を②のスナップショットと比較し、変わっていれば `mergeBundle(base=②, local=現在, remote=merged)` で再度3-wayしてから⑤で反映する**（同期中に変わったキーだけ現在の値が勝ち、他は `merged` のまま）。その差分はまだDriveに無いので **`_lastSnap` は「書き込んだ内容(json)」にして、次の自動同期で必ず push させる**。※`await` を挟んで `store.data` を差し替える処理を足す時は、同じ「後で現在値と再マージ」を必ず入れる。
...
- `app.js` 冒頭の `APP_VERSION`＝**`v{YYYYMMDD}-{HHMM}`（JST更新日時）**。コミットのたびに更新し、`index.html` の `?v=` も同じ日時に揃える（運用ルールは CLAUDE.md ルール8）。表示＝**マスタ画面ヘッダ右**。すみぽんが実機のキャッシュ鮮度を確認する手段。
...
### 16.7.4 保存容量パネルと techAnalysis の再肥大（2026-08-18）
症状: 銘柄追加のティッカー自動取得が「取得失敗」。実体は**取得は成功しており localStorage が上限で保存だけ失敗**していた（`autoFetchInfo` が fetch と `store.setMeta` を同じ try で囲っていたため区別できていなかった）。取得と保存を分離し、文言を「取得失敗（通信・再試行可）」「取得OK・保存失敗（容量不足の可能性）」に出し分け、通信失敗は600ms後に1回だけ自動リトライする。

**マスタ →「保存容量（ブラウザ）」（`openStorageMaster`）**: `store.data` のキー別サイズ／**localStorage 全キー**のサイズ／**techAnalysis のフィールド別内訳**を表示し、「再取得できるキャッシュを整理」「テクニカル分析の結果を消す」「ニュース一覧も消す」「同期の基準を作り直す」を実行できる。**0バイトでも必ず結果をトーストで返す**（`_freeCacheSpace()` は登録外の分析結果しか消さないため、登録銘柄ばかりの環境では何も起きず「ボタンが壊れている」ように見えた）。

実機計測（2026-08-18）: アプリ 3,088KB のうち **techAnalysis 2,358KB（76%）**、サイト全体 4,954KB＝上限約5MBの**97%**。差分の約1,865KB は **`sm_sync_base`**（＝`dataBundle()` 全体のコピー）で、**データが大きいと容量を実質2倍使う**。

1銘柄あたりの実測（AAPL・3年日足）: 分析結果本体 9.7KB（metrics 3.2 / marks 2.5 / levels 1.9 / patterns 0.7 / evidence 0.6KB）＋ **`history` 382B/点**。history は1日1点たまるので52点で**19KB**＝1銘柄の大半を占める。
- **浮動小数の丸め `techRound()`**: 保存時に小数を4桁へ丸める（`344.57000732421875`→`344.57`）。実測**20%削減**。`saveTechResult` と `rescoreFromMetrics` の両方に適用。
...
- **再取得間隔の段階化（2026-07-27）**: 従来は決算が3ヶ月先でも**全銘柄を毎日取り直して**いた（1銘柄あたり JP=2 / US=2〜5 サブリクエスト）。`earnRefetchInterval(sec)` が次回決算までの距離で間隔を決め、`at` からの経過日がそれ未満の銘柄はスキップする。
  - 直近発表（`prev` から7日以内）＝**毎日**（発表直後は次回予定日が確定・更新されやすい）
  - 確定予定日（`confirmedNext`）あり … **14日以内=毎日 / 15〜30日=3日 / 31日以上=7日**
  - **推定のみ**（`estNext`＝前回+決算間隔の憶測）… **28日以内=毎日 / 29日以上=3日**。確定日より毎日の窓を2週間広げるのは推定日が±2週間ズレるため。遠くても7日にしないのは、推定銘柄の再取得の主目的が「予定日」ではなく **`prev`（実際の発表日）の更新確認**で、「M/D発表」ラベルの窓が7日しかなく7日間隔だと発表を取り逃すから。
  - 推定が無効化（間隔判定ミス/不定期＝いつ来るか不明）… **3日**
  - 完全未取得・取得失敗 … 毎日（実際の再試行抑制は既存の `tryAt`＝60分バックオフが担う）
  - 間隔は毎回その時点の状態から計算するため、決算が近づけば自動的に毎日へ戻る。確定予定日が取れた推定銘柄も自動で確定側のルールに移る。
...
- **銘柄名の横のラベル `earnLabel`**（**一覧の表だけ**・近い/直近のときだけ）:
  ...
  - 予定が近い/過ぎて間もない: 確定予定日の**1週間前〜7日後** → 「M/D予定」(琥珀)
  - 推定が近い/過ぎて間もない: 推定日(前回+決算間隔)の**2週間前〜7日後** → 「M/Dごろ」(灰) ＋ **前回決算日を併記**（`前回M/D`）
  - 「いつまで」を3分岐とも7日後に統一した経緯は §20.3.1（2026-07-29）
...
### 20.3.1 決算日の空白期間（2026-07-29 すみぽん決定）
**症状**: 決算予定日を過ぎるとラベルが何も出なくなり、詳細でも次回決算が `-`・前回決算は前四半期のまま。

**原因（実測で確認）**: ソース側の反映が発表に間に合わない。`/api/earnings` を叩くと、前日に発表した銘柄
（例 V=2026-07-28発表）は `next`＝過ぎた日付のまま・`prev`＝2026-04-28（前四半期）で返る。
US は Nasdaq の Date Reported（earnings-surprise）が発表から**1〜2日遅れ**、JP は Yahoo が
**発表と同時に予定日を消す**。この隙間で「予定日=過去orなし / 前回=古い」が同時に起きる。
（20260727 の再取得の段階化が原因ではない。予定日が近い/過ぎた銘柄は段階化後も毎日取得。）
そのうえで **ラベルの3分岐が全部外れて空白**になっていた: ①発表=前回決算日基準（古いので出ない）
②予定=予定日基準（消えたので出ない）③推定=「推定日の当日まで」で切れる。

**ラベルの表示期間（`earnLabel`）**: 「いつから」＝決算をまたぐかの判断に使う期間、
「いつまで」＝株価が動いた理由として決算を疑える期間。**いつまでは3分岐とも「7日後まで」で統一**する。

| 優先 | 種類 | 表示 | 期間 | 赤強調 |
| --- | --- | --- | --- | --- |
| 1 | 発表済み `earn-done` | `7/27発表` | 前回決算日の当日〜**7日後** | 当日・前営業日 |
| 2 | 予定 `earn-soon` | `7/27予定` | 予定日の**7日前〜7日後** | 前営業日・当日・翌営業日 |
| 3 | 推定 `earn-est` | `7/27ごろ` | 推定日の**14日前〜7日後**（サブに前回決算日） | なし（不確実なので） |

**方針として採らなかったもの**: 過ぎた予定日を「発表済み（見込み）」として前回決算に繰り上げる案は、
**取得できていない前回決算を推測で埋める**ことになるため却下（2026-07-29）。推定日を過ぎたら次の期へ
繰り越す案も、取得できていないのに次の予定日を出すことになるため却下。**前回決算日は取得できた値のみ**。

**列「次回決算」・上部表示（`earnEffNext`）はラベルと同じ日付・同じ区分にする**（2026-07-29 すみぽん指示）:
予定日を取得できていれば「予定」、無ければ 前回+サイクル の「ごろ」。予定日を過ぎても
**発表日をまだ取得できていない間（7日以内）はその予定日を「予定」で出す**（ラベルが `7/27予定` の時に
列が `7/27ごろ` になるズレを無くす）。発表日を取得できたら（prev ≧ 予定日）その予定日は過去なので推定「ごろ」へ。

| 状態 | ラベル | 前回決算欄 | 次回決算列 |
| --- | --- | --- | --- |
| 予定日あり(7/27)・前回未取得 | `7/27予定` | 2026/4/27 | `2026/7/27 予定` |
| 予定日が消えた・前回未取得 | `7/27ごろ` | 2026/4/27 | `2026/7/27 ごろ` |
| 前回7/27取得・次の予定日なし | `7/27発表` | 2026/7/27 | `2026/10/27 ごろ` |
| 前回7/27取得・次の予定日あり | `7/27発表` | 2026/7/27 | `2026/10/27 予定` |
| 予定日から8日経過・前回未取得 | なし | 2026/4/21 | `2026/7/21 ごろ`（31日で `-`） |

**あわせて入れた保護**:
- `earnInfo` の `confirmedNext`（＝「次回」として出す予定日）は**未来（当日含む）のものだけ**。過ぎた予定日は
  次回ではないので外し、ラベル用に生の値 `schedule` として返す（ラベル②が過ぎて7日まで使う）。
- `refreshEarnings` の保存を**マージ**に変更: `next` は新しい値が無ければ既存を残す（ソースが消しても
  ラベル②の根拠が残る）。`prev`/`prev2` は**新しい prev が既存より新しい時だけ組で入れ替える**
  （部分取得で古い値に巻き戻さない・null で消さない）。
- 列「次回決算」の推定は従来どおり**推定日から31日で無効化**（ラベルより長く残す。すみぽん判断）。
...
「株価（・5年高値）を取得中… n/m件」（バッチ完了ごとに加算）→「5年高値を取得中… n/m件」→「銘柄情報を取得中… n/m件」→「ランキング順位を取得中…」。
※2026-08-31 以降はオーバーレイに出るのは「株価を取得中… n/m件」のみ。後続の重い段階は 20.6 のバックグラウンド進捗バーに移行。

### 20.5 決算日取得のバックグラウンド化（2026-08-27 改善）
...
### 20.6 株価以外の重い更新を全部バックグラウンド化（2026-08-31 すみぽん指示）
価格更新の前面（busy オーバーレイ）で待たせるのは**株価取得のみ**にし、時間のかかる更新はすべて 20.5 の決算日と同方式のバックグラウンド＋トップバー下の進捗バーに移した。従来は初回/日次（`withHighs=true`）に5年日足込みの重い取得を前面で行い、件数表示が進まず止まって見えていた。

- `refreshAll` の前面処理: 株価（BATCH=20・highs 無し）＋為替/指数＋前日終値＋米株時間外のみ。`withHighs` オプションは廃止（引数は無視）。
- バックグラウンド（`api.bgTask` の**直列キュー**で1つずつ実行。並行させない＝サブリクエスト上限と進捗バーの取り合い回避）:
  1. **5年/52週高値** `refreshHighsBg` … 銘柄単位の成功日 `highsAt !== today()` で対象選別（highsFail バックオフは従来どおり）。10件ずつ `refreshPrice`（highs=1）。完了で `lastHighsDate` を更新。
  2. **銘柄情報**（名前未取得の銘柄のみ）
  3. **決算日** `refreshEarningsBg`（従来どおり）
  4. **ランキング順位バッジ** `loadRankBadges`
...
- **API**: `functions/api/company.js` … `GET /api/company?symbols=7203.T,AAPL` → `{SYM:{summary}}`。
  JP=`kabutan.jp/stock/?code=XXXX` / US=`us.kabutan.jp/stocks/TICKER` を取得し、`概要</th>` 直後の `<td>` を
  タグ除去して返す（実測で日米とも同じ構造・400字で切る）。1銘柄=1サブリクエスト、クライアントは8件ずつ。
- **保存**: `store.data.meta[priceKey].summary` ＋ `summaryAt`（銘柄マスタ＝Google同期対象。meta は updatedAt の新しい方が勝つ）。
- **取得タイミング**（すみぽん確認 2026-07-29）:
  - 通常（日次更新）… **未取得の銘柄だけ**取りに行く。既に持っていれば0リクエスト
  - 「銘柄情報を更新（名前・セクター・PER等）」＝**洗い替え時に全銘柄を取り直す**（`refreshCompany(secs,{force:true})`）
...
- **端数処理は次回購入（買い増しライン）と共通**（2026-08-01 すみぽん指示）。`floorBuyPrice(v, market)` に集約し、
  `calc.evaluate` のトリガー丸めと目標株価の両方から呼ぶ＝米株は1ドル単位（10ドル未満は0.1ドル）・日本株は円未満切捨て。
  表示書式も買い増しラインに合わせる（カルテ/ドロワー＝`m()`、表の列＝`fmtAmt`）。
...
- **前回購入は「買い増し判定」ボックス**の基準値の直下に置く（保有状況ではなく判定の基準として読む値のため）。
- **保有ボックス**: 合計の数量・平均取得単価は口座別行と同じ `数量 @ 単価` 書式。**口座が1つなら合計行は出さない**（口座別行と同内容になるため）。
- **0株の口座は表示しない**（カルテ・詳細ドロワーとも `quantity > 1e-9` のみ）。ただし**保有の編集フォーム**（0株ロットを編集・削除するため）と**汎用エクスポート**（取込↔出力の往復で行を失わないため）は0株も対象のまま。
- **余白**: `.kt` に負マージンを入れて `main.content` の上下 padding を打ち消す。上端の空きは検索欄の下の余白（`--sp-2`＝6px）と揃える。検索欄はラベルを持たず placeholder のみ、「表示」ボタンは入力欄のすぐ右。
- **フォーム共通（`form .row`）**: 左右の列は `.field` を縦フレックスにしてラベル側を伸ばし、入力欄を下端で揃える。`select` と `input[type=date]` は Chrome が `line-height` を無視して `input` より2px高くなるため `height:40px` を明示（左右のガタつき防止）。

### 21.6.1 カルテ上部の保有サマリ（2026-08-04 追加）
株価・前日比の右に **取得額 / 評価額 / 損益率** を出す。**原通貨のみ**（米国株＝ドル、日本株＝円）。
円換算の併記は保有ボックス（米国株は「ドル / 円」の2列）に任せ、ヘッダーでは通貨を混ぜない（`.kt-pos` / `posCell`）。

## 21.8 全売却（2026-08-05 改訂）
全売却は **証券口座（証券会社×口座）単位**で行える。実体は `recordSellAll(secId, hids)`＝対象ロットごとに
現在値（未取得なら平均取得単価）で「売り」取引を記録して数量を0にする。`hids` 省略時は全口座。

- **確認は専用モーダル**（`openSellAllModal`）。売却内容（証券会社/口座・数量・売値・概算売却額と選択中合計）を表で見せ、
  **チェックボックスで口座を選べる**。既定は全口座チェック済み（1口座を指定して開いた場合はその口座のみ）。
- **入口3つ**: 保有の編集フォームの行ごと「全売却」（その口座のみ選択状態で開く）／同フォーム下部の「全売却（口座を選択）」／
  **銘柄詳細ドロワーのフッター「全売却」**（保有がある時だけ表示）。
- 一覧の「選択を全売却」（`bulkSellAll`）は従来どおり銘柄単位（複数銘柄・全口座）で、確認は `confirm`。

## 21.9 取引履歴・ランキング登録・画面の並び（2026-08-05 追加）
- **取引履歴は `txnHistoryHtml(secId)` に共通化**（編集✎・削除×つき。カルテと同じ `.kt-txn-row` 行UI）。
  **取引記録フォーム（`openTxnForm`。新規記録時のみ）**と**銘柄詳細ドロワー**の「購入・取引履歴」で共用する。
  詳細から「取引」を開いた先に履歴が無く、過去の取引を確認・修正できなかったため（2026-08-05 すみぽん指摘）。
  取引フォームから✎編集した場合は、更新後にその銘柄の取引フォーム（履歴つき）へ戻る。
- **マーケットのランキングからの登録は注意銘柄(watch)にしない**（`addRankingWatch`。ボタンは「＋登録」）。
  注意フラグは自分で警戒する銘柄に付けるもので、ランキングからの登録＝単なる銘柄追加のため（2026-08-05 すみぽん指示）。
- **左メニューの画面（タブ）の並びをマスタで変更可**（マスタ・設定 →「画面（タブ）の並び順」＝`openNavOrderMaster`）。
...
## 21.5 取引の既定 証券会社（2026-07-29 追加）
取引記録フォーム（`openTxnForm`）の証券会社の初期値は「実際の保有（数量が多いロット優先）→ 直近の買い」の順で決まるが、
**未保有かつ取引履歴が無い銘柄**は情報が無く `BROKERS[0]`（SBI）固定だった。市場ごとの既定をマスタで持つ。
- 保存: `store.data.settings.defaultBroker = { JP, US }`（**Googleドライブ同期の対象**。sync SCHEMA は `settings:['singleTs']` なので追加登録不要。保存時に `settings._updatedAt` を更新）
- 既定値: `DEFAULT_NEW_BROKER = { JP:'楽天', US:'Webull' }`（未設定時・BROKERS に無い値のときのフォールバック）
- UI: マスタ・設定 →「取引の既定 証券会社（未保有の銘柄）」（`openDefaultBrokerMaster`）
- 判定は**現時点の保有株数**で行う（2026-07-29 すみぽん指示）: 株数>0の口座があればその証券会社（数量が多いロット優先）、**株数が0なら常にこの既定**。過去の取引履歴・数量0の保有レコードは見ない（全部売った銘柄を買い直す時は新しく買う先が既定になる）。
- 口座種別の既定は従来どおり（保有ロット→数量0のレコード→ACCOUNTS[0]）
- 銘柄追加フォームの「初期保有」・保有フォームの「保有を追加」は従来どおり BROKERS の先頭（今回の対象外）

## 21. 資産に占める割合（比率列）（2026-07-28 追加）

保有1銘柄が資産全体のどれくらいを占めるかを、**評価額比率 / 取得額比率**の2指標で表示する。比較はすべて**円換算**で行う（米株は為替、為替未取得時は「マスタ・設定 → ドル円換算レート」で代替。分母から欠落させないため）。

### 21.1 分母の選択（4種）
`store.data.settings.ratioDenom` に保存（**Google同期対象**。sync SCHEMA は `settings: ['singleTs']` のため追加登録は不要。変更時は `settings._updatedAt` を更新する）。

| 値 | 表示名 | 分母 |
|---|---|---|
| `both`（既定） | 日米合計 | 登録済み全銘柄（US+JP）の合計 |
| `shown` | 表示中の合計 | 検索・列フィルタ適用後にその表に出ている行の合計 |
| `market` | 市場別（日/米） | **行が属する市場**の合計（日本株の行→日本株合計・米国株の行→米国株合計） |
| `manual` | 手入力額 | 手入力の金額。`ratioManualAmount` ＋ `ratioManualCcy`（`JPY`/`USD`。USDは共通レートで円換算） |

### 21.2 実装
- 描画直前に `ratioCtx = buildRatioCtx(表示中の銘柄)` をセットし、`COL_RENDERERS.ratioValue/ratioCost` がそこから分母を引く（`_render()` の先頭で日米合計を既定セット→表を持つ画面が上書き）。`shown` の母数は**ソート前・フィルタ後**の集合で確定させる（比率ソートも同じ母数で行うため）。
- 列: `ratioValue`（評価額比率）/ `ratioCost`（取得額比率）。MASTER_COLS・COL_RENDERERS・`sortValue`・`colDefaultWidth`・`cfCellValue`／`CF_NUMERIC_KEYS`（背景色ルール可）に配線。既定表示＝保有銘柄(US/JP)・買い増しサイン。**自動算出の派生値なので取込（GI/汎用往復）・一括変更・転記の配線はしない**（手入力する項目ではないため）。
- 分母の切替UIは保有銘柄一覧・買い増しサインのツールバー（`ratioToolbarHtml()`）。分析タブは列設定で出せるが切替UIは持たず、設定値に従う。
- 詳細ドロワーには「資産に占める割合」行（`評価額% / 取得額%（分母: ラベル）`）を表示。分母ラベルを併記して誤読を防ぐ。
- **銘柄カルテは評価額ベース・分母＝日米合計に固定**（`karteRatioHtml`。ツールバーの分母設定に依存しない）。取得額比率は分母（全銘柄の取得額合計）と分子の時点がそろわず「資産に占める割合」として解釈しづらいため出さない（2026-07-29 すみぽん指示）。
- 表示は小数第2位まで（例 12.34%）。セルは％テキストの背後に構成比バー（`.ratio-td .ratio-bar`）を敷く。tooltip に「分子 ÷ 分母（円）」を出す。

---

## 22. マクロ指標タブ（FRED・2026-08-13 追加）

インフレ・金利・SLOOS（FRBが全米の銀行の融資担当者に行う調査）・景気雇用・信用の指標を、
カード＋グラフ＋表で一覧する。**値・変化・過去比の事実表示に徹し、売買の推奨は出さない。**

### 22.1 データ源と取得（`functions/api/macro.js`）
- 出典は **FRED®（セントルイス連邦準備銀行）**。返却に `Source: FRED®, Federal Reserve Bank of St. Louis` を含め、画面下部にも出典を表示する（FRED の利用条件）。
- 経路は2本立て。`GET /api/macro?ids=A,B,C&start=YYYY-MM-DD` で最大30系列。
  1. `FRED_API_KEY`（Cloudflare Pages の環境変数）があれば公式API `api.stlouisfed.org/fred/series/observations`
  2. 未設定なら **`fredgraph.csv`（キー不要の公開CSV）へ自動フォールバック**。公式API側が失敗した時も CSV で1回リトライする
  → キー発行前でも動き、キーを入れると自動で公式API経路になる（コード変更不要）
- エッジキャッシュ 6時間（`cf.cacheTtl`）。FRED の更新は日次〜四半期なので十分。
- 系列IDは外部URLにそのまま乗るため `^[A-Z0-9]{2,32}$` で厳格に検証する。
- **返す観測値は間引き済み**（`THIN_MAX=320` 点／直近 `THIN_RECENT=200` 点は生のまま・それ以前を等間隔サンプリング）。
  日次系列を15年ぶん生で返すと `store.data.macro`（＝Google同期対象）が肥大するため。25系列で約105KB に収まる。
- 頻度（日次/週次/月次/四半期）は日付間隔の中央値から推定し、表示ラベルにのみ使う（判定には使わない）。

### 22.2 系列と表示変換（`MACRO_SERIES` / `MACRO_GROUPS`）
- 25系列。すべて fredgraph で実在と直近値を確認済み（2026-08-13）。
- `tf` で表示変換する: `raw` / `yoy`（約1年前の観測に対する変化率%。日付でいちばん近い過去の点を探し許容60日）/ `diff`（前期差）。
  CPI・PCE・鉱工業生産・小売売上は指数のままでは読めないので `yoy`、非農業雇用は `diff`（前月差）にする。
- `good`（`up`/`down`）＝その方向に動くと景気・金融環境にとって追い風、という向き。前回比・1年差の**色分けにのみ**使う（良し悪しの断定はしない）。
- グループ（画面のセグメント）は インフレ / 金利 / 融資動向(SLOOS) / 景気・雇用 / 信用・リスク の5つ。
  1カード＝1グラフで、`ids` が複数なら同じグラフに重ねる。**重ねるのは単位が揃う組み合わせだけ**（例: FF金利・2年債・10年債）。
- SLOOS はプラス＝「貸出基準を厳格化した」と答えた銀行が多い（純％）。0をまたぐかが意味を持つのでゼロ線を濃く引く。
- 廃番に注意: `DRTSPM`（住宅ローン基準）は2014年で系列終了のため不採用。住宅ローンは `SUBLPDHMSGNQ`（GSE適格）を使う。

### 22.3 保存と同期
- 取得結果は `store.data.macro`（系列ID→`{obs, freq, at}`）。**`sync-merge.js` の SCHEMA に `macro: ['map', byAt]` を登録済み**（系列ごとに取得時刻 `at` の新しい方。ルール7-2のキー単位マージ）。
- 表示状態（グループ・カード・期間）は `settings.macroGroup` / `macroCard` / `macroPeriod`。`settings` は `keyedTs` なのでキー単位で混ざる（ルール7）。
- 取得は「更新」ボタン＋**セッション中1回だけの自動取得**（`_macroTried`）。
  ★`_macroTried` が無いと「取得失敗→再描画→また自動取得」の**無限リトライ**になる（実装中に実際に発生）。

### 22.4 収まり（ルール6）
グラフ高さは **「暫定260pxで描く→実レイアウトを測って描き直す」を最大3回** で決める（`renderMacroChart` / `macroChartFitH`）。
一発計算にできない理由と、必ず引くもの:
- 初回描画では表（`.table-wrap`）がまだ高さ0で測れることがあり、一発計算だと必ず大きすぎる値になる（実測 338px vs 正解 182px）。
- **`main.content` 自身の下パディング（実測53px）を引く**。忘れると毎回その分ページがスクロールする。
- **`.section` の `margin-bottom`（20px）を足す**。`getBoundingClientRect` には含まれないが `scrollHeight` には効くため、落とすと約16px はみ出したままになる。
- 検証は必ず `main.content.scrollHeight - clientHeight` で行う（`document` 基準は body が `overflow:hidden` のため常に0で誤判定）。全5グループ×5期間＝25通りで はみ出し0 を確認済み。

### 22.5 ローカル開発時の制約
**ローカル wrangler（workerd）から `stlouisfed.org` へ到達できない**（接続がタイムアウトする。ホストの curl では0.2秒で200が返るので FRED 側の問題ではない）。
既知の「ローカルでは Yahoo quoteSummary がブロックされる」と同種のローカル限定制約。
そのためローカルでのUI確認は、ホスト側で取得したCSVを `/api/macro` と同じ形に整形して `store.data.macro` に流し込んで行う。

### 22.6 変化の色は「符号」ではなく「指標ごとの向き」（2026-08-13 すみぽん指摘への対応）
前回比・1年差の色は `macroDeltaCls` が系列の `good`（up/down）に沿って付ける。株価の前日比（+緑/−赤）とは違い、
**同じマイナスでも指標によって緑と赤に分かれる**（CPI −0.2＝インフレ鈍化で緑 / 非農業雇用 −87＝雇用減速で赤）。

すみぽんから「符号が同じなのに色が違うのが気になる」と指摘があり、**色のルールは変えず、反転する理由を画面で読めるように**した:
- `macroGoodMark(id)` … 指標名の横に `↓良`（下がるほど追い風）/ `↑良`（上がるほど追い風）のチップを出す。カードと表の両方。
- `macroDeltaTitle(d, id)` … 変化の数値の tooltip に「この変化は追い風/向かい風の向き」と言葉でも出す。
- 表の下に凡例1行（`色: 緑＝望ましい向きに動いた／赤＝逆向き（↓良…・↑良…）。符号は増減そのもの。`）。
  ★凡例は**1行に収める**こと。4行の説明文にしたら「景気・雇用」グループで103pxはみ出した（グラフが最小180pxで吸収できないため）。

**符号そのままの色（+緑/−赤）にしなかった理由**: 失業率 +0.3 や VIX +2.0 が緑になり誤読を招くため。
マクロ指標は「上がった＝良い」が指標ごとに逆転する。

### 22.7 指数・為替との「比較」（2026-08-13 追加・同日に方式を2回改訂）
S&P500・NASDAQ100・日経平均・TOPIX(1306.T)・ドル円を、いまのグラフに重ねて比較できる。
**目的は「指数が暴落・回復したタイミングで、その指標がどう動いたか／相関があるか」を見ること。**

- 指数は `MACRO_SERIES` に **`src:'idx'`** として同居させ、描画・表・ツールチップは FRED 系列と同じ経路を通す。
  取得元だけ分岐する（`/api/history`＝Yahoo）。保存は `store.data.macroIdx`（SCHEMA `['map', byAt]` 登録済み）。
- 10年日足は約2,500点あるので `macroThin()`（直近200点＋それ以前を等間隔・計320点）で間引く。5系列で約49KB。
- **選択は指標（カード）ごとに保持**する（`settings.macroCompare` のキーは `グループ/カード`）。
  「CPIには日経を重ねたいが、VIXには重ねたくない」のように見たい組み合わせは指標ごとに違うため。
  UIはニュースの開示トグルと同じ `seg seg-toggle`。「すべて」「全解除」は置かず、1つずつ選択・解除する。

#### 表示方式の変遷（同じ失敗を繰り返さないための記録）
1. **起点=100 の指数化（不採用）** … 全系列に同じ倍率を当てるため、日経が+138%動く隣で CPI（1〜3%）が
   横一線になり、指標がまったく見えない。
2. **各系列を0〜100に正規化（不採用）** … 形とタイミングは比べられるが、**元の指標の実値がグラフから読めない**。
3. **2軸（採用）** … **左軸＝指標の実値（実線）／右軸＝指数の実値（破線）**。実値を保ったまま両方の振れが見える。
   - 右軸に回すのは「比較で追加した指数」＋カードが `right:[...]` で指定した系列
     （日経66,970とTOPIX連動ETF432のように、同じ軸に置くと片方が潰れる組み合わせ）。
   - **比較の指数を2つ以上選んだ時だけ右軸を「起点=100」に切り替える**（日経66,970とS&P7,752を実値で
     同じ右軸に置くと片方が画面の4%まで潰れる）。左軸＝指標は実値のままなので「指標の値が読めない」問題は起きない。
     実値は凡例とカーソルで確認できる。
   - 凡例に**相関係数 r**（カードの1本目を基準）。頻度が違う系列（CPI月次×日経日次）は、点の少ない方の各日付に
     「その日以前で最も新しい相手の値」を当てて対にする（`macroCorr`）。共通期間10点未満なら出さない。
     r は水準どうしの相関なので、両方が上昇トレンドというだけで高く出る。因果の証拠ではない旨を tooltip に明記。

#### 軸スケールの決め方（`scaleTo` / `naturalN`）
左右で目盛り本数を揃えつつ、軸をデータへ最大限詰める。ここで2回失敗している:
- 目盛り本数を固定して `lo + step*TICKS` で上限を決めると、あと少し足りない時に刻みが1段階粗くなって軸が大きく余る
  （実測: 日経1.7万〜7.2万に対し軸0〜120,000。振れが画面の36%まで潰れた）。→ **上限は切り上げ**にする。
- 本数を揃える時に粗い側の軸を上へ伸ばすと、その軸が余る（実測: ドル円100〜162に対し軸100〜240＝span35%）。
  → 伸ばさず、**本数が targetN に収まる最小の刻みを選び直す**（`stepCands` は 1/2/2.5/5×10^n。共有の
  `niceStep` より細かい）。修正後 span 69%。
- 収まり調整で `macroLineChart` は同じ series で最大4回呼ばれる。右軸の指数化は **`s.raw` があれば再変換しない**
  ガードが必須（無いと2回目以降で指数値をさらに指数化し、凡例の実値が 66,970→396 に壊れる）。
### 22.8 更新タイミング（2026-08-13）
- 手動「更新」ボタン（いつでも可）
- **1日1回の自動取得**: マクロ指標タブを表示した時、`settings.lastMacroDate !== todayJst()` なら1回だけ取得。
  1件でも成功したらその日は済み扱い。全滅した日は次にタブを開いた時にまた試す。
  セッション中の再試行は `_macroTried` で1回に制限する（失敗時の無限リトライ防止）。
- 株価更新（`dailyStartup`）とは独立。銘柄を1件も登録していなくてもマクロだけは更新される。

### 22.9 収まりの追い込み（ルール6・2026-08-13）
「重ねる」チップ行が増えてツールバーが3段になり、「景気・雇用」（カード6枚＋表7行）で最大156pxはみ出した。
2段構えで吸収する:
1. グラフの下限を 180px → **150px**
2. それでも余る分は **表を枠内スクロール**にする（`macroFitTable`。下限110px）
   - この枠には `.macro-managed` を付け、**汎用の `fitListTables` の対象から外す**（二重管理でちらつくため）。
     `fitListTables` は「縮めた結果160px未満になるなら何もしない」ので、この画面では専用処理が要る。

全6グループ × 重ね合わせ有無＝12通りで `main.content` のはみ出し 0 を確認済み。

### 22.10 日本の指標（2026-08-13 追加）
「日本」グループ。**FRED の日本CPIは全系列が 2021-06 で更新停止**しているため、CPI だけ e-Stat から取る。

| 指標 | 取得元 | 備考 |
|---|---|---|
| 日本CPI 総合 / コア / コアコア | e-Stat `0004052037`（消費者物価指数 2025年基準） | `cdTab=3` が前年同月比(%)なので `tf:'raw'`。品目 `cdCat01` は 0001総合 / 0161生鮮食品を除く総合 / 0178生鮮食品及びエネルギーを除く総合。`cdArea=00000` 全国 |
| 日本10年国債 / 短期金利(コール) | FRED `IRLTLT01JPM156N` / `IRSTCI01JPM156N` | 月次 |
| 日本 失業率 | FRED `LRHUTTTTJPM156S` | 月次 |
| 日本 実質GDP(前年比) | FRED `JPNRGDPEXP` | 四半期・水準なので `tf:'yoy'` |
| 日本 輸出額(前年比) | FRED `XTEXVA01JPM667S` | 月次・水準なので `tf:'yoy'` |
| 円 実質実効為替レート | FRED `RBJPBIS` | 下落＝円安。輸出には追い風・輸入には逆風で一概に良し悪しが言えないため `good` を付けない＝色付けしない |

**未搭載**: 鉱工業生産（FREDは2024-03で停止／e-Statの現行基準の表が検索で出てこない）、日銀短観（e-Statに無い。日銀の時系列データサイトが本体）。

#### e-Stat 実装の注意（`functions/api/estat.js`）
- **statsDataId は推測しない**。`op=list`（表の検索）→ `op=meta`（分類コード）で本番のキーを使って実物を確認してから配線する。
- `getStatsList` に **`statsNameList=Y` を付けてはいけない**。統計表(`TABLE_INF`)ではなく統計名(`LIST_INF`)が返り、表が0件になる。
- 時間軸コード（10桁）は `YYYY + 期間種別(2) + 期(2) + 期(2)`。月次=種別`00`・四半期=種別`02`・年度=`10`。
  **年次・年度の行は月次系列に混ぜない**（`2025100000`＝2025年度が2025年1月として紛れ込み頻度が壊れる）。解釈できない種別は落とす。
- 同じ時点の値が重複したら**エラーにする**（分類の絞り込み不足を黙って合算しないため）。
- 出典表記は取得元ごとに必要。画面下部の出典は表示中の系列の取得元だけを並べる（`macroSrcNote`）。

### 22.11 「時間が経つと選択が元に戻る」の根本原因と対策（2026-08-13）
症状: マクロ指標で「日本」を開いていたのに、しばらくすると「インフレ」に戻る。
以前に報告された「タブ移動したのに時間が経つと違うタブに戻される」と**同じ根本原因**。

**原因: 複数タブ／複数ウィンドウで、古い内容を持ったままのタブが押し戻していた。**
`store.data` は**タブごとのメモリ**にあるのに、同期の基準 `sm_sync_base` は localStorage で**全タブ共有**。

1. タブAで「日本」に変更 → 同期（Drive=`jp` / base=`jp`）
2. 開きっぱなしのタブBのメモリは `infl` のまま
3. タブBが同期すると `base=jp / local(B)=infl / remote=jp` → keyedTs は「rj===bj」なので
   **「タブBが意図的に infl へ変更した」と判定**して local を採用 → `infl` が Drive に push される
4. タブAが次の同期で `infl` を取り込み、選択が元に戻る

※3-wayマージ自体は正しい。2端末シナリオ・往復中の変更（f26fc72）とも検証したが巻き戻らなかった。
  悪いのは「タブが古い内容を持ち続けること」。

**対策**: `storage` イベント（変更したタブ**以外**で発火）で `store.load()` し直し、古い内容を持ち続けないようにする。
入力中・モーダル/ドロワー表示中は再描画を見送り、データだけ取り込む（打鍵中の内容を消さないため）。
マクロに限らず、設定・タブ・列設定で起きていた同じ症状に効く。

**あわせて実施（ルール7-2 違反の是正）**:
- マクロの表示状態を `settings` から独立キー **`macroView`（`keyedTs`）** に分離。
  `settings` は Google連携・比率分母・マトリックス等が共有する1つのオブジェクトで、`_updatedAt` は
  「どれか1つでも変わった時刻」。そこへ同居させると他機能・他端末の書き込みがマクロの選択を巻き込む。
- 自動書き込みの `lastMacroDate` も `settings` から独立キー（`max`）へ。
  **自動取得・派生キャッシュを settings に入れない**（ルール7-2）。旧データからは起動時に1回だけ引っ越す。
- `macro` / `macroIdx`（約190KB）を容量逼迫時に捨てられる「再取得できるキャッシュ」に登録（`_freeCacheSpace`）。

## 23. 更新情報フィード（NEWバッジ／ダッシュボードのお知らせ・2026-08-14 追加）

「経済指標が更新された」「YouTube動画が上がった」を1本のフィードに貯め、
サイドナビの **NEWバッジ** と、ダッシュボードの **「更新情報」一覧**（企業サイトのニュースリリース風）で見せる。

### 23.1 データ
- `store.data.updates`（配列・最大80件）… `{ id, key, kind:'macro'|'video', title, sub, date, at, ... }`
  - `kind='macro'`: `seriesId` / `group` / `card`（クリックでその指標へ遷移するため）
  - `kind='video'`: `link` / `videoId`
  - 同じ `key` は上書き（同じ指標の再更新で行が増えない）。SCHEMA は `['records', r => 'upd:'+r.key]`
- `store.data.updSeen`（key→既読日時）… SCHEMA は `['map', null]`。端末をまたいで既読が揃う
- **並び順は「出来事の日付(`date`)」の降順**（指標＝観測日 / 動画＝公開日）。同日なら `at` で降順（`updCmp`）。
  ★一度「気づいた時刻(`at`)」で並べたが、まとめて取得した指標が全部同じ時刻になって順不同に見えるうえ、
  「7月分のCPI」と「3月公開の動画」が取得順で混ざり時系列にならなかった（2026-08-14 すみぽん指摘）。
  一覧の右に出す日付も `date` を優先する（`at` を出すと7月分のCPIが「13:05」と表示され時系列に見えない）。

### 23.2 裏取得のタイミング（起動時＋価格更新＋ニュース更新）
★これが無いと更新情報は「マクロ指標タブを開いた後」「ニュースで更新を押した後」にしか増えず、
**ダッシュボードに出す意味が無い**（2026-08-14 すみぽん指摘）。起動時に `updDailyCheck()` を呼び、
マクロ指標と購読YouTubeの新着を裏で確認する。

- **ニュース一覧（`newsPool`）には触らない**。「開いたら勝手に更新される」のを止めた経緯があるため
  （2026-07-23 すみぽん要望）。ここで取るのは購読YouTubeの新着だけ（`/api/youtube` を1回）。
- **起動時のほか「価格更新」「ニュース更新」を押した時にも走る**（2026-08-14 すみぽん要望）。
  外部への負荷が増えないことを確認済み:
  - YouTube は**公式RSS（APIキー不要・クォータ無し）**。さらに `/api/youtube` が Cloudflare で10分キャッシュ。
  - FRED は6時間エッジキャッシュ。押し直しても同じURLはキャッシュから返る。
  - Cloudflare Pages Functions の無料枠は10万リクエスト/日で、1回の更新で十数リクエスト。桁が違う。
- ガード: マクロは**日付**（`lastMacroDate`。月次・四半期・週次の指標が1日に何度も変わることはない）、
  動画は**10分のクールダウン**（`lastVideoCheck` は日付ではなく時刻を保存。旧データの 'YYYY-MM-DD' も
  `Date.parse` で扱えるので移行不要）。どちらも独立キー＋`max`マージ。
- ボタン起点（`manual=true`）のときは、セッション内で一度失敗していてもマクロを再挑戦する。
- ニュース更新は自前で動画を取っているので、その中で `lastVideoCheck` を更新して直後の二重取得を防ぐ。
  逆にニュース取得が失敗した場合は `lastVideoCheck` が更新されないため、末尾の `updDailyCheck(true)` が
  取りに行く（取りこぼさない）。
- **マクロと動画は並列に走らせる**。直列（await）にすると、マクロが遅い時に動画の確認が始まらない
  （実測: ローカルでFREDに繋がらず90秒以上ブロックされ動画が取れなかった）。
  終わったものから順に画面へ反映する。
- プールに無い動画でも、更新情報の記録（title/videoId/link）から `openVideoPanel` を開ける。

### 23.2.1 検知
- **経済指標**: `macroRefresh` / `macroFetchEstat` で保存する**直前**に、取得前の最終観測日と比べて
  新しい観測が入った時だけ積む（`updNoteMacro`）。**初回取得では積まない**（prevLastDate が無い＝全系列が
  一斉にNEWになるのを防ぐ）。指数・為替（`src:'idx'`）は毎日動くので対象外。
  **さらに日次の指標（米国債利回り・FF金利・期待インフレ・VIX等の7系列）も積まない**。毎営業日に値が入るので
  お知らせに出すと毎日流れて他の更新が埋もれる（2026-08-14 すみぽん指摘）。月次・四半期・週次のみ対象。
- **YouTube**: `newsRefresh` の `videoItems` から、まだフィードに無い videoId を積む（`updNoteVideos`）。

### 23.3 既読とバッジ
- ナビの `badge: 'new:macro'` / `'new:video'` は件数ではなく「NEW」を出す。
- ニュース（動画）は**そのタブを開いたら既読**（`go()` 内）。一覧の1件クリックでもその1件を既読にする。
- ダッシュボードの「すべて既読」で全消し。
- **マクロ（経済指標）だけは「開いた瞬間に既読」にしない**（2026-09-18 すみぽん指摘: タブにNEWは付くが
  中を開くとどれが更新されたのか分からない）。開いた瞬間に既読にすると、そのときの新着が一切表示に残らないため。
  マクロ指標タブを**表示している間はNEWを出したまま**にし、**タブを離れる時に既読へ移す**
  （`go()`: `if (currentView === 'macro' && view !== 'macro') updMarkSeen('macro')`）。
  タブ内の「すべて既読」ボタンで即座に消すこともできる。

### 23.3-b マクロ指標タブ内の「どれが更新されたか」（2026-09-18）
未読のマクロ更新を 系列ID→レコード に引く `macroUnseenMap()` を用意し、`renderMacro` で4か所に印を出す:
- **新着バー**（`.macro-newbar`・未読がある時だけ表示）: 「NEW 更新 n件」＋**指標名＋観測日のチップ**（`.macro-newchip`）。
  チップのクリック `macroOpenNew(seriesId)` で**その指標のグループ／カードへ切り替え、その1件だけ既読**にする。
  右端に「すべて既読」（`macroMarkNewSeen()`）。
- **グループのセグメント**・**カード**・**表の行**（`.macro-newdot` / `tr.macro-row-new`）に NEW の印。
  グループ内・カード内のどれかに未読があれば印を出すので、別グループの更新も一目で分かる。
- 収まり（ルール6）: バーは1行（約39px）。グラフ高さの自動補正（`macroChartFitH`）と `macroFitTable` が
  実測で吸収するため、窓が十分にあれば `main.content` のはみ出しは 0（実測: 1280×860 で overflow 0）。

### 23.4 クリックの遷移
- 経済指標 → `macroView` にグループ／カードを書いてから `go('macro')`＝その指標が選ばれた状態で開く。
- 動画 → `go('news')` してニュース一覧の該当行までスクロール＋ハイライトし、**その行をクリックした状態にする**
  （`el.click()`＝既読にして動画の要約パネルを開く）。一覧に出ていない場合もプールに記事があれば
  `openVideoPanel` で直接開く。
  - ★行の特定に **`CSS.escape` を使った属性セレクタは使わない**。`CSS.escape` は識別子用で、
    URL が `https\:\/\/…` にエスケープされて引用符付き属性値と一致しない（実際に見つけられなかった）。
    `[...document.querySelectorAll('.news-item')].find(a => a.dataset.link === link)` で素直に比較する。
  - 一覧に無い場合（古くてプールの保持期間外など）は**外部タブを勝手に開かない**でトーストで知らせる
    （ポップアップ扱いで塞がれるうえ、不意に別サイトが開くため）。

### 23.5 収まり（ルール6）
ダッシュボードは元々ちょうど窓いっぱい（実測: 余り5px）。12件そのまま出すと310px溢れたので:
- 見出しは `section-head`（それだけで60px）を使わず `upd-head`（約24px）にする
- 一覧の高さは `fitUpdList()` が残りスペースに合わせる（最大300px・最小84px＝約2行、超過分は枠内スクロール）。
  ★対象は **`#upd-feed-list`（ID指定）**。マクロ警告カードも `.upd-list` を使うので、クラスだけで探すと
  そちらを縮めてしまい更新情報が縮まない（実測でダッシュボードが305pxはみ出した）。
それでも窓が低い時はダッシュボード自体が少しスクロールする。**元から満杯の画面に追加している以上、
ここは「無駄な余白」ではなく実コンテンツによるスクロール**として許容する。

### 23.6 サイドナビのアイコン重複解消（2026-08-14）
`マーケット`＝`report`（レポートと同じ棒グラフ）、`マクロ指標`＝`signal`（買い増しサインと同じ折れ線）が
重複していた。**後から追加したタブ側**を変更（すみぽん指示）:
- マーケット → `podium`（ランキングの表彰台）
- マクロ指標 → `gauge`（景気の温度計）
全14タブでアイコンの重複が無いことを確認済み。

## 24. マクロ指標の基準値ライン・警告（2026-08-14 追加）

指標ごとに「基準値」と「どちらへ抜けたら警告か」を登録し、グラフの水平線・ダッシュボード・定時通知メールで知らせる。
**設定した条件を満たした事実を伝えるだけで、売買の推奨はしない**（画面・メール本文にも明記）。

### 24.1 データ
`store.data.macroAlerts` = `[{ id, seriesId, mode, cmp, value, enabled, updatedAt, deleted }]`
- `mode`: `level`（実値）/ `chg`（前期差）/ `yoy`（1年差）
- `cmp`: `gt`（上回ったら）/ `lt`（下回ったら）
- SCHEMA は `['records', r => 'ma:'+r.id]`。**削除はトンボストン**（`deleted:true`＋新しい `updatedAt`）。
  配列から消すと同期で復活する（ルール5-8）。

### 24.2 判定
`macroAlertState(a)` … **画面と同じ値（前年比などの変換後）** で判定するため `macroPoints()` を通す。
- `level`=最新値 / `chg`=最新−1つ前 / `yoy`=最新−約1年前（`macroAtBefore` で許容60日）
- データ不足なら `null` を返し「データ不足」と表示（黙って条件外にしない）

### 24.3 表示
- **グラフの水平線**: `level` の警告だけ描く（`chg`/`yoy` は「差」なので値の軸に乗らない）。
  左軸に出ている系列のもののみ、かつ軸の範囲内のときだけ（枠外に線が出るのを防ぐ）。
  成立中は赤・条件外は琥珀の破線＋「基準 ○○」ラベル。
  ★**基準値は左軸の範囲に必ず含める**（`boundsOf(list, extra)`）。データの範囲外に基準値があると線が引けず、
  期間を変えただけで設定した線が消えて見える（実測: コアPCEの基準2.0が5年表示の軸2.5〜6.0から外れて消えた）。
- **ダッシュボード**: 「マクロ警告」カード。**未設定なら何も出さない**（空枠で場所を取らない）。
  成立行はクリックでその指標へ遷移。
- **マクロ指標タブのヘッダ**に「基準値・警告（成立件数）」ボタン → 一覧・追加・有効/無効・削除のモーダル。

### 24.4 メール通知（`functions/lib/macro-alerts.js`）
定時通知（`notify-run`）に「マクロ警告」欄を追加。
- **値はその場で `/api/macro`・`/api/estat` から取り直す**。端末が同期していなくても最新で判定できる。
- 系列定義（変換規則・ラベル・単位）はサーバー側にも持つ。クライアントに無い系列IDの警告は黙って飛ばす
  （片方だけ系列が増えた時に落ちないように）。
- **サインが0件でもマクロ警告があればメールを送る**（そのための警告設定なので）。件名にも件数を足す。
- 取得失敗は best-effort（通知全体は止めない）。レスポンスに `macroAlerts:{fired,checked,skipped,error}` を返すので
  `/api/notify-run`（送信なし）で動作確認できる。

## 25. AI相場診断（Gemini・2026-08-28 追加）

買い増しサインタブの「🤖 AI相場診断」ボタン。ツール内のデータをGeminiに渡し、
「相場環境／買い候補の優先順位／見送り・注意／全体への注意」の4部構成の診断文を返す。
...
- `app.js aiDiagPayload()`: 送信データの組立。**金額（円/ドルの絶対額）・数量は送らない**（2026-08-28 すみぽん決定）。
  ...
  ニュース見出し25件（本文なし）。
  サイン銘柄ごとの内訳: 格付・評価（＋analysisAsOf=分析日。古い分析はAIが割り引く）・優先順位・現在値・
  PER/PBR/配当利回り・時価総額・目標指標からの逆算株価と乖離%（targets.gapPct）・テクニカル（RSI/52週乖離/
  200日線/MACD/パターン。asof付き）・5年高値/前回購入からの下落率・残り下落率・比率%・シナリオ位置・
  決算まで日数・銘柄別関連ニュース見出し（newsSecHit で照合・3件まで）・会社概要（株探 meta.summary 120字）・分析メモ200字。
  評価項目は「株価連動か非連動か」で分ける（2026-08-28 すみぽん指示）:
  **送らない=★バリュエーション・買い時評価（buyGrade）・総合評価（overallGrade）**（分析時点の株価で買い時まで
  加味した判断のため、株価が動くと古くなり誤誘導する）。
  **送る=格付（rating）・★独自の強み（moatStars）・★リスク（riskSafetyStars）**（企業の質の評価・株価非連動）＋分析メモ。
  ★リスクは**★が多いほどリスクが低い（安全）**という向きなので、キー名を `riskSafetyStars` とし
  プロンプトでも向きを明示する（Geminiが逆に読み「★4＝リスク高」と誤警告した実績あり・2026-08-28）。
  プロンプトで「格付・★が高いから買い、の理由付けは禁止。質と割安かは分けて論じ、買い時は現在のPER等から自ら評価」と指示。
  プロンプト側で「銘柄ごとに割安/割高の見立てを必ず述べる」「事業内容・下落背景は一般知識で補足可（『一般知識では』と明示）」を指示。
- 結果は `store.data.aiDiag = {at, text, model, _updatedAt}` に保存し全端末で共有（ルール7）。
  sync-merge SCHEMA は **singleTs**（診断1回=1つのスナップショットの塊のため。newsPool と同じ扱い）。
- 無料枠の学習利用を踏まえ、プライバシー方針（金額を送らない）は API 側コメントとモーダルの注記にも明記。
...
- 次回購入ラインの算出に**使ったルールの下落率**だけを「−20%」のように表示する列 `ruleDrop`（`ruleDropInfo(sec, ev)`）。残り下落率とは別。基準（高値から／前回から）と内訳は tooltip のみ。買増固定値は「—」（2026-09-18 すみぽん指示）。
- `calc.evaluate` の分岐（`baseSource`）とそろえる: 初回・高値更新後（`rule.highResetMode` ON のルールだけ＝適用区分「高」）・買い増しも初回基準 → 初回下落率「高値から」／買い増し → 買い増し下落率「前回から」（前回購入単価が無く高値基準なら「高値から」）／買増固定値・判定外 → 「—」。**初回ルール適用が OFF のルールは高値更新後も買い増し扱い**（evaluate がそう判定するため自動でそうなる）。
- tooltip に「基準値 → −％ → 次回購入（ルール名）」。markets=US/JP/SIGNAL、既定表示（適用区分の隣）、`sortValue`（固定値・判定外は末尾）／`cfCellValue`（表示どおり負の％）＋`CF_NUMERIC_KEYS`／`colDefaultWidth`=76。ルール由来の派生値なのでフォーム・取込・一括変更は対象外。
- **列設定の新列の挿入位置を変更**（`reconcileColPrefs`）: 保存済みの列設定に無い新列は、従来は末尾に追加していたが、`MASTER_COLS` 上で直前にある列の直後に挿入する（関連列の隣に出るように）。

## 28. 決算カレンダー（2026-09-25 追加・kessan-tool から移植）
サイドナビ「決算カレンダー」（view `ecal`）。日本株・米国株の決算発表の予定日を2か月のカレンダーで見る（スマホ幅は1か月）。日付を押すとその日の一覧。

### 28.1 取り先（`functions/api/calendar.js`。kessan-tool と同じ実装。キャッシュキーのみ `sm-cache.local`）
| 市場 | 取り先 | 呼び方 |
|---|---|---|
| 米国 | Nasdaq 決算カレンダー（非公開の窓口） | `?market=us&month=YYYY-MM`（月の平日を1日ずつ） |
| 日本 | JPX「決算発表予定日」の Excel（全部） | `?market=jp&src=jpx` |
| 日本 | 日経「決算発表スケジュール」（50件/ページ） | `?market=jp&month=YYYY-MM&hm=1`（残りは `next`） |
- JPX は 3月期・9月期の会社を翌営業日分しか載せないので日経でおぎなう。同じ会社が45日以内に JPX にあれば日経の行は捨てる。
- 結果は Cloudflare のキャッシュに6時間（一部の日が読めなかった時は置かない）。鍵を使わないので api-guard の対象外。
- 画面側はメモリだけに持つ（`ECAL`）。永続化しない。

- `?names=SYM,...`（最大12）：米国株の日本語名（Yahoo!ファイナンス日本版の og:title。日本語を含まなければ null）。銘柄ごとに30日キャッシュ（一時的な失敗は覚えない）。画面は一覧の上から200行ぶんだけ、12銘柄×同時3本で取りに行き、取れた分から描き直す（セッション中はメモリ `ECAL.ja`）。

### 28.2 このツール独自の機能
- 行の銘柄を銘柄マスタ（同じ市場の ticker）と突き合わせ、「保有」「登録」の印を付ける。銘柄名クリックで銘柄詳細（`openSecurityDetail`）。コードは株探へ。
  - 突き合わせは英数字だけで比較（BRK.B＝BRK/B）、JPX の5桁コード（72030）は4桁に。
- 絞り込み：すべて／登録銘柄／保有中＋検索欄。登録・保有中・検索中は2か月ぶん全部を一覧、「すべて」は選んだ日だけ。
- カレンダーの ★n＝その日の登録銘柄の件数。
- フィードに無い登録銘柄は、既存の決算日キャッシュ `store.data.earnings[priceKey].next`（/api/earnings）でおぎなう（前後20日以内にフィードの行があれば足さない）。行に「（予想）/（決算日）」と表示。

- 日付の選び方（2026-09-25 すみぽん指摘で改修）：状態は `ECAL.from/to`（1日なら同じ日、'' は表示中の2か月ぶん全部）。
  - クリック＝その日。同じ1日をもう一度＝全日付。Shift+クリック／なぞる＝期間。一覧見出しの「期間」入力欄（スマホ用）。←→＝1日・↑↓＝1週送り。**0件の日（今の絞り込み・検索で）は飛ばし**、同じ向きに次の決算日まで進む（2026-09-25 すみぽん指示）。表示中の2か月に無ければ1か月送って読み込み後に探す（`ECAL.jump`）。見つからなければ元の月に戻して「この先／この前に決算予定がありません」。入力中・モーダル中は無効。
  - どの絞り込み（すべて／登録銘柄／保有中）でも日付・期間・全日付が使える。全日付の時だけ「過去も表示」チェック（既定は今日以降のみ）。
- 一覧の列：`MASTER_COLS` に `ec*` キー（markets は `ECAL_US`/`ECAL_JP` のみ）を足し、保有銘柄と同じ列設定（`colPrefs` の ECAL_US/ECAL_JP スコープ・「列」ボタン＝`openColPicker`）で表示・並び・幅・列名を変える。見出しクリックで並べ替え（`listState.ECAL_*`。同じ値の中は登録銘柄→米国は時価総額順／日本はコード順）。描画・並べ替えの値は `ECAL_COL`。
- リンク：コード＝登録銘柄は右の銘柄詳細、未登録は株探。銘柄名＝株探。
- 右の銘柄詳細は「重ねるだけ」（`body.drawer-modeless`）：背景を暗くせず（overlay は pointer-events:none）後ろの一覧・カレンダーも押せる。幅900px以上は本文の右にドロワーぶん（min(480px,40vw)）の余白を空ける。閉じる・画面移動（go）でクラスを外す。表示中の行は青。

### 28.3 設定（ルール7）
- `store.data.settings.ecalMarket`（'JP'|'US'）・`ecalScope`（'all'|'mine'|'held'）。settings は keyedTs なので SCHEMA 追加は不要。

### 28.4 収まり（ルール6）
- 一覧は `.section .table-wrap` なので `fitListTables()` が枠内スクロールにする。検索欄は作り直さず（`ecalPaint` がカレンダー・一覧だけ描き直す）、打鍵・変換中のフォーカスを保つ。
