...
- `earnInfo`: `prev`（実績）/ `confirmedNext`（Yahoo確定の予定日＝`next && !nextEstimate`）/ `estNext`（予定日が無ければ **前回+3ヶ月** を推定）。
- **銘柄名の横／詳細上部のラベル `earnLabel`**（近い/直近のときだけ）:
  - 直近発表: 前回発表から**7日以内** → 「M/D発表」(緑)
  - 予定が近い: 確定予定日の**1週間前〜当日** → 「M/D予定」(琥珀)
  - 推定が近い: 推定日(前回+3ヶ月)の**2週間前〜** → 「M/Dごろ」(灰) ＋ **前回決算日を併記**（`前回M/D`）
- **ラベル（ピル）は一覧の表だけ**（`COL_RENDERERS.name` の銘柄名の横）。詳細ドロワー・カルテではラベル形式にしない。
- **詳細ドロワー上部・カルテ上部＝株探風の日付テキスト `earnTopHtml`**（ラベル/ピルではなくプレーンな日付。「決算 前回 2026/5/8 ／ 次回 2026/8/4（予定）」。ドロワーは格付けの右に右寄せ、カルテは「取引を記録」ボタンの左）。次回は 予定（確定）/ ごろ（推定）を併記。
...
- **列**: `earnPrev`(前回決算) / `earnNext`(次回決算) を選択可能な列として追加（MASTER_COLS/COL_RENDERERS/**sortValue**/colDefaultWidth）。既定表示には入れない（列設定で任意に出す）。
  - `earnNext` 列は**予定日が無ければ推定（前回+4ヶ月）も含めた実効日 `earnEffNext`** を「YYYY/M/D 予定|ごろ (あとN日)」で表示し、**`sortValue` で実効日の近い順にソート可能**（未取得は `9999-99-99` で末尾）。前回の確定のみ版はソートに使えなかったため実効日ベースへ変更。
