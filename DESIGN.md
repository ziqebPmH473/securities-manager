...
- **○の色（2026-07-23 確定）**: 未読＝青（既定）のうち、**「更新」で入った新着だけ黄**。**黄は次にタブを開き直した時点で青へ戻る**（`_newsSeenMark`＝タブに入り直した時のプール取得時刻。`batch===at && at!==_newsSeenMark` が黄。go() のニュース入場時とリロード後初回表示でマーク更新）。既読は従来表示のまま（グレー○等の追加なし）。緑の区別なし。pool の prevAt は保持するが表示には未使用。
