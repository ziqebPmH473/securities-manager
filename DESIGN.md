...
- **列「時価1位まで」＝市場の時価総額1位までの上昇余地（2026-08-06）**: 保有銘柄/銘柄マスタ/サインの列 `capToTop`。**同じ市場（日米別）の時価総額1位の時価総額 ÷ その銘柄の時価総額 − 1** を「◯.◯◯倍」で表示（例: 1位NVDA 4.99T・MU 1.00T → 3.99倍＝+399%の上昇余地）。1位自身は「1位」。分子（市場の時価総額1位）は `store.data.settings.mktTopCap = { US/JP: {code,name,cap,at} }` に保持（**原通貨の実額**。US=USD / JP=円。`calc.marketCap` は百万単位なので比較時に ×1e6）。更新経路は**追加取得ゼロ**の2つ——①株価更新時のランキング順位バッジ取得 `loadRankBadges`（`kind=marketcap&sub=all&count=10` を日米とも1日1回取得済み）の1位、②マーケットタブで時価総額ランキング（全市場）を開いた時の `loadRanking`。`settings` 配下なので Google同期対象（ルール7。更新時に `_updatedAt` を打つ＝singleTs）。1位が未取得のうちは「—」（tooltip で取得方法を案内）。**配線**: `MASTER_COLS`／`COL_RENDERERS`／`DEFAULT_VISIBLE`(US/JP)／`sortValue`／`cfCellValue`＋`CF_NUMERIC_KEYS`（背景色ルール可・倍率なので `CF_MONEY_KEYS` には入れない）／`colDefaultWidth`／銘柄詳細ドロワー。**自動算出の派生値なので、銘柄編集フォーム・取込（GI/汎用往復）・一括変更・転記用には通さない**（手入力する項目ではないため）。
