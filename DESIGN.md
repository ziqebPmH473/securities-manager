...
- **並び順(`at`)は「気づいた時刻」で統一する**。動画だけ公開日時にすると経済指標（検知時刻）と時計が混ざり、
  新着動画が古い扱いで一覧の下に沈む（実測で先頭12件に出てこなかった）。公開日は `date` として別に表示。
...
- 動画 → `go('news')` してニュース一覧の該当行までスクロールし一瞬ハイライト（`.upd-flash`）。
...
- 一覧の高さは `fitUpdList()` が残りスペースに合わせる（最大300px・最小120px＝約3行、超過分は枠内スクロール）
