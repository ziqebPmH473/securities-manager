...
### 15.1.7 ニュース翻訳の長文対応（2026-07-22 修正）
- **旧バグ**: `/api/translate` が length>1200 のテキストを **filter で除外**していた。①長い英語要約は翻訳が常に失敗（記事パネルを開くたび「翻訳を取得できませんでした」）②バッチ内の1件が落ちると `translated` の件数がズレ、クライアントの見出し/要約の対応付けが崩れて**別記事の訳が混ざり得た**。
- **修正**: 除外ではなく**先頭1500文字に切り詰めて翻訳**（サーバー・クライアント両方で切り詰め＝件数が常に一致）。長文要約も冒頭は翻訳される。ソース側の一時ブロック等による失敗は従来どおり5分後に再試行・原文フォールバック。
