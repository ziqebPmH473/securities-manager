...
- **○の色（3状態・2026-07-23 確定）**: 各記事に `batch`（初めて取得に現れた時刻。既存記事は前回の値を引き継ぐ）を刻み、`batch===at`（今回の取得で入った未読）＝**黄** / それ以外の未読＝**青**（既定） / **既読＝グレー**（`.news-item.read`）。**タブを開いても黄は増えず、「更新」すると前回の黄は青に落ちる**（前回/それ以前の区別＝緑は不要とすみぽん判断で廃止。pool の prevAt は保持するが表示には未使用）。
