...
- `app.js aiDiagPayload()`: 送信データの組立。**金額（円/ドルの絶対額）・数量は送らない**（2026-08-28 すみぽん決定）。
  ...
  **★評価（starValuation等）・買い時評価（buyGrade）・総合評価（overallGrade）は送らない**（いずれも「分析時点の
  株価で買い時か」まで加味した判断で、株価が動くと古くなり誤誘導するため。2026-08-28 すみぽん指示）。
  送る評価は格付（rating＝企業の質）のみで、プロンプトで「格付が高いから買い、という理由付けは禁止。
  買い時の妥当性は現在のPER等から自ら評価」と指示。
