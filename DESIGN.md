...
### 22.7 指数・為替の重ね合わせ比較（2026-08-13 追加）
S&P500・NASDAQ100・日経平均・TOPIX(1306.T)・ドル円を、いまのグラフに**重ねて比較**できる。
...
- 10年日足は約2,500点あるので、クライアント側で `macroThin()`（直近200点はそのまま＋それ以前を等間隔・計320点。
  `functions/api/macro.js` と同じ規則）に間引いてから保存する。5系列で約49KB。
- **指数化（起点=100）**: 重ね合わせを1つでも選ぶ／カードに `norm:true` があると、全系列を期間の起点=100に
  正規化して形だけを比べる。CPI 3.3% と 日経 67,500 を同じ軸に置くと形が読めないため。
  - 凡例の数値は**指数化後ではなく実数の最新値**＋起点比%（100.0が並んでも意味が読めないため）。
  - ツールチップも実数を出し、指数値は括弧で併記。
  - **0や負を含む系列は指数化しない**（前年比・SLOOS など。割り算で意味が壊れる）。重ね合わせから外し、
    黙って消えたと誤解されないよう注記に「指数化できず外した」と出す。
  - `norm:true` は桁の違う組み合わせのカードに付ける（日経66,970とドル円159を実数で重ねるとドル円が横一線になる）。
