...
### 20.3.1 「予定日を過ぎたのに前回決算が古いまま」への対処（2026-07-29）
**症状**: 決算予定日を過ぎると次回決算が消え、前回決算が前四半期のまま止まる銘柄が多数出る。

**原因（実測で確認）**: ソース側の反映が発表に間に合わないため。実際に `/api/earnings` を叩くと、
前日に発表した銘柄（例 V=2026-07-28発表）は `next` が過ぎた日付のまま・`prev` は 2026-04-28（前四半期）で返る。
US は Nasdaq の Date Reported（earnings-surprise）が実際の発表から**1〜2日遅れて**更新され、
JP は Yahoo が発表と同時に「次回決算発表予定日」を消す。この隙間で next=過去/なし・prev=古い、が同時に起きる。
（＝ 20260727 の再取得の段階化が原因ではない。段階化は予定日が近い/過ぎた銘柄では毎日取得のままになる。）

**対処**:
- `earnInfo` で、**過ぎた予定日（1〜45日前）を「発表済み（見込み）」として prev に繰り上げる**（`prevAssumed`）。
  次回＝その日+決算間隔で推定するので、次回決算が消えずに次の四半期を指す。本物の prev が来たら自動で置き換わる。
  45日を超えた過去予定日は誤りの可能性があるので使わない。
- 表示: 見込みの前回決算には注記（列は `*`、ドロワー/カルテは「（予定日・確報待ち）」）。
- `earnRefetchInterval`: 見込み中は**毎日**再取得（14日たっても確報が来なければ3日に落とす）。
- `refreshEarnings` の保存を**マージ**に変更: `next` は新しい値が無ければ既存を残す（Yahooが消しても見込み判定の根拠が残る）。
  `prev`/`prev2` は**新しい prev が既存より新しい時だけ組で入れ替える**（部分取得で古い値に巻き戻さない・null で消さない）。
