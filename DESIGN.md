...
- **3段フォールバック**: clients5（複数qを1リクエスト・URL6000文字単位でチャンク分割）→ gtx 1件ずつ（最大10件）→ **Gemini**（`env.GEMINI_API_KEY`＝動画要約と同じキー・gemini-3.1-flash-lite・JSON配列in/outで件数を厳密検証）。非公式エンドポイントがCFのIPをブロックしても翻訳が全滅しない。キー未設定時は従来どおり原文フォールバック。
